...
export const importRouter = router({
  ...
  importFromCsv: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const row of rows) {
        try {
          const creationDatePlus24h = new Date(new Date(row.qCreatedAt).getTime() + 24 * 60 * 60 * 1000)
          ...
          const parts = {
            ...
            resolution: row.resolution ?
              (row.resolution.toLowerCase().trim() === 'yes' ? Resolution.YES
                : (row.resolution.toLowerCase().trim() === 'no' ? Resolution.NO
                  : Resolution.AMBIGUOUS))
              : null,
            resolveBy: row.resolveBy ? new Date(row.resolveBy) : creationDatePlus24h,
            resolvedAt: row.resolution ?
              (row.resolvedAt?.trim() ? new Date(row.resolvedAt) : creationDatePlus24h)
              :
              null,
            notes: `${row.notes || ''}${row.notesExtra || ''} Imported from your spreadsheet on ${getDateTimeYYYYMMDDHHMMSS(new Date())}.`,
          }
          const forecastParts = {
      ...
      if (errs.length > 0) {
        throw new TRPCError({
          ...
          message: `We couldn't import ${errs.length} of your rows.\n`
          + errs.map(err =>
  ...
  deleteAllCsvQuestions: publicProcedure
    .mutation(async ({ ctx }) => {
      if (!ctx.userId) {
        throw new TRPCError({ code: 'UNAUTHORIZED' })
      }
