...
export async function buildResolveQuestionBlocks(
  teamId: string,
  ...
  return [
    {
      ...
      text:
        question.resolvedAt == null
          ? markdownBlock(
              `Hey ${getUserNameOrProfileLink(
                teamId,
                question.user,
              )}, you asked:\n ${questionTitle}\n You said it should resolve on ${resolutionDateStr}. How should this resolve?`,
            )
          : markdownBlock(
              `_You resolved ${questionTitle} as ${question.resolution}_`,
            ),
      ...
      elements: [
        ...(question.resolvedAt == null
          ? answerLabels.map((answer) => ({
              ...
              ...(answer != "ambiguous" && {
                style: answer == "yes" ? "primary" : "danger",
              }),
