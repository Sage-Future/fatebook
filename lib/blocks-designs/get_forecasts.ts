...
export async function buildGetForecastsBlocks(
  ...
  const lastPage =
    page == Math.ceil(latestForecasts.length / maxForecastsVisible) - 1
  ...
  const forecastsForPage = latestForecasts.slice(
    page * maxForecastsVisible,
    (page + 1) * maxForecastsVisible,
  )
...
function matchForecastsToScores(
  forecasts: Forecast[],
  questionScores: QuestionScore[],
) {
  const forecastScores = questionScores.filter((qs: QuestionScore) =>
    forecasts.find((f) => qs.questionId == f.questionId),
  )
...
async function buildGetForecastsBlocksPage(
  forecasts: ForecastWithQuestionWithQMessagesAndRMessagesAndForecasts[],
  questionScores: QuestionScore[],
): Promise<Blocks> {
          ...
          elements: [
            markdownBlock(
              await buildForecastQuestionText(
                forecast,
                questionScores.find(
                  (qs: QuestionScore) => qs.questionId == forecast.questionId,
                ),
                maxDecimalPlacesForecastForecastListing,
              ),
            ),
...
function getLatestForecastPerQuestion(
  forecasts: ForecastWithQuestionWithQMessagesAndRMessagesAndForecasts[],
) {
