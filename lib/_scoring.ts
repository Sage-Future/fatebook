...
export function relativeBrierScoring(
  ...
  for (let j = question.createdAt.getTime(); j < endDay; j = j + day) {
    ...
    const currentInterval =
      j + day < question.resolvedAt!.getTime()
        ? j + day
        : question.resolvedAt!.getTime()
    const startOfInterval =
      j < question.resolvedAt!.getTime() ? j : question.resolvedAt!.getTime()
    ...
    const mostRecentForecastBeforeThisIntervalById: ForecastArray =
      getMostRecentForecasts(sortedForecastsById, startOfInterval)
    // get all the forecasts for each user inbetween the current time interval and the previous one
    const forecastsOfCurrentIntervalbyId: DaysForecasts[] =
      sortedForecastsById.map(([id, sortedForecasts]) => {
        return {
          id: id,
          forecasts: sortedForecasts.filter(
            (f) =>
              f.createdAt.getTime() < currentInterval &&
              f.createdAt.getTime() >= startOfInterval,
          ),
          mostRecentForecast: mostRecentForecastBeforeThisIntervalById[id],
        }
      })

    const weightedForecastsOfCurrentIntervalById: DayAvgForecast[] =
      getWeightedAverageForecastOfInterval(
        forecastsOfCurrentIntervalbyId,
        startOfInterval,
        lengthOfInterval,
      )
    let absoluteBrierScores: [string, number | undefined][] =
      weightedForecastsOfCurrentIntervalById.map(({ id, avgForecast }) => {
    ...
    let medianBrierScore: number = median(
      absoluteBrierScores
        .map(([, score]) => score)
    ...
    let relativeBrierScores: [string, number | undefined][] =
      weightedForecastsOfCurrentIntervalById.map(({ id, avgForecast }) => {
        if (avgForecast === undefined) {
          return [id, undefined]
        } else {
          return [
            id,
            brierScore(avgForecast, question.resolution!) - medianBrierScore,
          ]
        }
      })
    ...
    if (relativeBrierScores.length != absoluteBrierScores.length) {
      throw new Error(
        "relative and absolute brier scores are not the same length",
      )
    }

    // add the score to the user score array
    for (let i = 0; i < relativeBrierScores.length; i++) {
      const [id, relativeScore] = <[string, number | undefined]>(
        relativeBrierScores[i]
      )
      const [, absoluteScore] = <[string, number | undefined]>(
        absoluteBrierScores[i]
      )
      if (
        relativeScoreTimeSeries[id] === undefined &&
  ...
  for (const id of Object.keys(relativeScoreTimeSeries)) {
    avgScoresPerUser[id] = {
      relativeBrierScore:
        uniqueIds.length === 1
          ? undefined
          : averageForScoreResolution(
              relativeScoreTimeSeries[id],
              Math.floor(days),
              fractionalDay,
            ),
      absoluteBrierScore: averageForScoreResolution(
        absoluteScoreTimeSeries[id],
        Math.floor(days),
        fractionalDay,
      ),
  ...
  if (uniqueIds.length != 1) {
    sortedIds = Object.keys(avgScoresPerUser)
      .map((id) => id)
      .sort(
        (a, b) =>
          avgScoresPerUser[a].relativeBrierScore! -
          avgScoresPerUser[b].relativeBrierScore!,
      )
  } else {
    sortedIds = uniqueIds
  }
...
function getWeightedAverageForecastOfInterval(
  forecastsOfCurrentIntervalbyId: DaysForecasts[],
  startOfInterval: number,
  day: number,
): DayAvgForecast[] {
  // mapping for each id
  return forecastsOfCurrentIntervalbyId.map(
    ({ id, forecasts, mostRecentForecast }) => {
      let avg = 0
      let sumOfWeights = 0
      ...
      if (mostRecentForecast != undefined && forecasts.length > 0) {
        ...
        const mostRecentWeight =
          (nextForecast.createdAt.getTime() - startOfInterval) / day
      ...
      for (let i = 0; i < forecasts.length - 1; i++) {
        ...
        const thisWeight =
          (nextForecast.createdAt.getTime() -
            thisForecast.createdAt.getTime()) /
          day
      ...
      avg =
        avg +
        lastForecastWeight * forecasts[forecasts.length - 1].forecast.toNumber()
      return { id, avgForecast: avg }
    },
  )
}

function getMostRecentForecasts(
  sortedForecastsById: [string, Forecast[]][],
  startOfInterval: number,
): ForecastArray {
  ...
  for (const idEntry of sortedForecastsById) {
    ...
    const forecastsBeforeCurrentInterval = sortedForecasts.filter(
      (f) => f.createdAt.getTime() <= startOfInterval,
    )

    if (forecastsBeforeCurrentInterval.length > 0) {
      mostRecentForecasts[id] =
        forecastsBeforeCurrentInterval[
          forecastsBeforeCurrentInterval.length - 1
        ].forecast.toNumber()
    } else {
...
function averageForScoreResolution(
  scores: number[],
  days: number,
  ...
  if (days == 0) {
    ...
    sum =
      scores.slice(0, -1).reduce((a, b) => a + b, 0) +
      scores.slice(-1)[0] * fractionalDay
  } else {
    divisor = days
    sum = scores.reduce((a, b) => a + b, 0)
  }
