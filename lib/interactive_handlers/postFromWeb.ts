...
import prisma, { deleteMessage, getOrCreateProfile, postEphemeralTextMessage, postSlackMessage } from '../_utils_server'
...
export async function postFromWeb(relativePath: string, teamId?: string, channelId?: string, slackUserId?: string) {
  ...
  if (relativePath.startsWith("/q/")) {
  ...
  } else if (["tournament", "predict-your-year"].some(path => relativePath.startsWith(`/${path}/`))) {
    const id = getQuestionIdFromUrl(relativePath)
    ...
    if (tournament.syncToSlackChannelId || tournament.syncToSlackTeamId) {
      return await postEphemeralTextMessage(teamId, channelId, slackUserId, `That tournament is already synced to a Slack channel, <#${tournament.syncToSlackChannelId}>.`)
    }
    ...
    await postSlackMessage(teamId, {
      channel: channelId,
      text: `<@${slackUserId}> has started synced their list to this channel: *<${getUserListUrl(userList, false)}|${userList.name}>*. Forecasting questions shared with that list will be posted here.`,
    }, slackUserId)
    console.log("initial question sync")
    ...
    await postEphemeralTextMessage(teamId, channelId, slackUserId, "We couldn't find a question, tournament or list at that URL.")
  }
}


// jank: removeFromEntities should still have their channel and team ids set, so that we can delete messages from them
export async function syncToSlackIfNeeded(question: QuestionWithTournamentsAndLists, userId: string | undefined, removeFromEntitiesThatStillHaveSlackIds?: (UserList | Tournament)[]) {
  for (const entity of [...question.tournaments, ...question.sharedWithLists]) {
    if (entity.syncToSlackChannelId && entity.syncToSlackTeamId) {
      const user = await prisma.user.findUnique({ where: { id: userId || "NO MATCH" } })
      if (user) {
  ...
  for (const entity of removeFromEntitiesThatStillHaveSlackIds || []) {
    if (entity.syncToSlackChannelId && entity.syncToSlackTeamId) {
      ...
      for (const slackMessage of slackMessages) {
        await deleteMessage(slackMessage.message.teamId, slackMessage.message.channel, slackMessage.message.ts)
...
export async function postQuestionToSlack({ questionId, teamId, channelId, user, slackUserId }: { questionId: string; teamId: string; channelId: string; user: User; slackUserId?: string; }) {
