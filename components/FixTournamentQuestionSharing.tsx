...
export function FixTournamentQuestionSharing({
  ...
  const unsharedWithList = tournament.questions.filter(
    (q) =>
      tournament.userListId &&
      !q.sharedWithLists?.find((l) => l.id === tournament.userListId) &&
      !q.sharedPublicly,
  )
  ...
  const shareWithList = api.userList.setQuestionLists.useMutation({
    onSuccess(data, { questionId }) {
      ...
      if (tournament.userListId)
        void utils.userList.get.invalidate({ id: tournament.userListId })
    },
  })
      ...
      <div className="flex flex-col gap-4">
        {[
          ...
          },
          {
            ...
            title: `questions in this tournament aren't shared with your team '${
              tournament.userList?.name || ""
            }'`,
          ...
          ({ questions, title, handleShare, expand, setExpand }) =>
            questions.length > 0 && (
              <div
                key={title}
                className="flex flex-col items-start space-y-2 bg-neutral-50 px-6 py-4 outline outline-neutral-200 outline-1 text-sm rounded-lg mt-4"
              >
                <div className="flex justify-between items-center w-full">
                  <div>
                    ...
                    {questions.length === 1 ? (
                      <button
                        className="btn"
                        role="button"
                        onClick={() => handleShare(questions[0].id)}
                        disabled={
                          sharePublicly.isLoading || shareWithList.isLoading
                        }
                      >
                        Share question{" "}
                        <i>
                          {questions[0].title.length > 40
                            ? `${questions[0].title.substring(0, 40)}...`
                            : questions[0].title}
                        </i>
                      </button>
                    ) : (
                      <button
                        ...
                        onClick={() =>
                          questions.forEach(
                            (q) => q.userId === userId && handleShare(q.id),
                          )
                        }
                        disabled={
                          sharePublicly.isLoading || shareWithList.isLoading
                  ...
                  <button
                    className="btn btn-xs p-2 mt-2 text-neutral-900"
                    role="button"
                ...
                {expand && (
                  <ul className="space-y-2 p-2 w-full bg-white rounded-md">
                    {questions.map((q) => (
                      ...
                      >
                        <Link
                          className="no-underline hover:underline my-auto"
                          href={getQuestionUrl(q)}
                        >
                          {q.title}
                        </Link>
                        <button
                          className="btn my-auto shrink"
                          role="button"
                          onClick={() => handleShare(q.id)}
                          disabled={
                            sharePublicly.isLoading ||
                            shareWithList.isLoading ||
                            q.userId !== userId
                          }
                        >
                          {q.userId === userId
                            ? "Share"
                            : "Can't share (you're not the author)"}
                        </button>
                      </li>
                    ))}
                  </ul>
