...
export function SyncToSlack({
  ...
  url: string,
  listType: string
  entity: {syncToSlackTeamId: string | null, syncToSlackChannelId: string | null, [key: string]: any}
}) {
    ...
    return (
      <div className="text-sm">
        <span className="text-neutral-500 italic">
          <Image src="/slack-logo.svg" width={30} height={30} className="m-0 -mx-2 mr-1 inline" alt="" />
  ...
  return (
    <div className="text-sm">
      <button className="btn" onClick={handleShareToSlack}>
        ...
        Sync this {listType}{"'s"} questions to Slack
        {showInstructions && <>
          <br/>{" (command copied!)"}
        </>}
      </button>
      {showInstructions && (
        <div className="mt-2 bg-neutral-100 rounded-md p-4 relative max-w-lg">
          <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" onClick={() => setShowInstructions(false)}>
            <XMarkIcon className="w-6 h-6 text-neutral-500" />
          </button>
          <span className="font-semibold">
            Paste into a Slack channel to share these questions there - and any questions added in the future
          </span>
