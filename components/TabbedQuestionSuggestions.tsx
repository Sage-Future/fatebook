  ...
  return (
    <div className="max-w-full">
      ...
      <QuestionSuggestionsByCategory
        ...
        suggestions={
          teamMode ? teamSuggestions(year) : personalSuggestions(year)
        }
        onSuggestionSelect={(title) => {
          setQuestionDrafts((drafts: any) => [
            ...drafts,
            { key: generateRandomId(), defaultTitle: title },
          ])
        }}
      />
    </div>
  ...
  return (
    <div className="flex flex-col gap-2 bg-white rounded-xl shadow-sm py-4">
      ...
      {selectedCategory !== null && (
        <motion.div className="w-full rounded-b-md px-6 pt-2 pb-4 mb-6 flex flex-col items-start gap-2 z-10">
          {suggestions
            ...
            ?.questions.map((question, index) => (
              ...
              >
                {question.label && (
                  <div className="bg-neutral-50 px-2 py-1 rounded-lg w-full">
                    <ReactMarkdown className="text-sm ml-4 pr-4 italic text-neutral-500 w-full">
                      {question.label}
                    </ReactMarkdown>
                  </div>
                )}
                  ...
                  >
                    <span className="mr-auto">
                      {clickedSuggestions.includes(question.q) ? (
                        ...
                        <span className="text-neutral-500 font-semibold mr-2 -ml-4">
                          +
                        </span>
                      )}
