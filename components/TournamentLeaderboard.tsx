...
export function TournamentLeaderboard({
  ...
  const scoresByPlayer = (() => {
    ...
    const resolvedQuestionScores = tournamentQ.data.questions
      .flatMap((question) =>
      ...
      .map((player: any) => ({
        ...player,
        absoluteScore:
          player.questionsWithAbsoluteScore > 0
            ? player.absoluteScore / player.questionsWithAbsoluteScore
            : undefined,
        relativeScore:
          player.questionsWithRelativeScore > 0
            ? player.relativeScore / player.questionsWithRelativeScore
            : undefined,
      }))

    const allUniquePlayersWhoSubmittedAtLeastOneForecast =
      tournamentQ.data?.questions
        .flatMap((q) => q.forecasts)
        .map((forecast) => ({
    ...
    return allUniquePlayersWhoSubmittedAtLeastOneForecast.map((p) => {
      const scores = resolvedQuestionScores.find(
      ...
      return {
        ...
        numForecasts: tournamentQ.data?.questions.reduce(
          (acc, q) =>
            acc + q.forecasts.filter((f) => f.userId === p.userId).length,
          0,
        ),
        numQuestions:
        ...
        <table className="table w-full py-2">
          <thead className="sticky top-0 bg-white">
            <tr>
              ...
              >
                Player{sortIndicator("user")}
              </th>
              <th
                className="cursor-pointer"
                onClick={() => requestSort("absoluteScore")}
              >
                Brier
                <br />
                score {sortIndicator("absoluteScore")}
                <InfoButton
              ...
              <th
                className="cursor-pointer"
                onClick={() => requestSort("relativeScore")}
              >
                Relative
                <br />
                Brier {sortIndicator("relativeScore")}
                <InfoButton
                  className="font-normal ml-1"
                  tooltip="Lower is better! Relative to the median on each question"
                />
              </th>
              <th
                className="cursor-pointer"
                onClick={() => requestSort("numQuestions")}
              >
                Questions
                <br />
                forecasted
              ...
              <th
                className="cursor-pointer"
                onClick={() => requestSort("numForecasts")}
              >
                Total
                <br />
                forecasts{sortIndicator("numForecasts")}
              </th>
            </tr>
          </thead>
          <tbody>
            {ifEmpty(
              sortedScores.map((score, index) => (
                <tr key={score.userId}>
                  <td className="text-xs text-neutral-400 font-semibold pr-0">
                    {sortConfig.direction !==
                    defaultDirectionForKey(sortConfig.key)
                      ? sortedScores.length - index
                      : index + 1}
                  </td>
