...
const createQuestionSchema = z.object({
  apiKey: z.string().nonempty({ message: 'apiKey must be a string (see https://fatebook.io/api-setup)' }),
  title: z.string().nonempty({ message: 'title must be a string' }),
  resolveBy: z.string().nonempty().refine(value => !isNaN(Date.parse(value)), {
    message: 'resolveBy must be a date string (YYYY-MM-DD)',
  }),
  forecast: z.string().or(z.number()).optional()
  ...
  shareWithLists: z.string().or(z.array(z.string())).optional()
    .transform((val) => typeof val === 'string' ? [val] : val),
  hideForecastsUntil: z.string().optional().refine(value => !value || !isNaN(Date.parse(value)), {
    message: 'hideForecastsUntil must be a date string (YYYY-MM-DD)',
  }),
  sharePublicly: z.string().optional().transform(value => value !== 'false' && value !== undefined),
  shareWithEmail: z.string().or(z.array(z.string())).optional()
    .transform((val) => typeof val === 'string' ? [val] : val),
})
...
const createQuestionPublicApi = async (req: Request, res: NextApiResponse) => {
  ...
  if (!result.success) {
    res.status(400).send(generateErrorMessage(result.error.issues, {
  ...
  const { apiKey, title, resolveBy, forecast, tags, shareWithLists, hideForecastsUntil, sharePublicly, shareWithEmail } = result.data
  ...
  if (shareWithEmail && shareWithEmail.length > 0) {
    ...
    const missingUsers = shareWithEmail.filter(email => !sharedWithUsers.some(sharedUser => sharedUser.email === email))
  ...
  const question = await prisma.question.create({
    data: {
      ...
      tags: tags && tags.length > 0
        ? {
            connectOrCreate: tags.map((tag) => ({
              where: {
                id: tagEntities.find((t) => t.name === tag)?.id || "no tag with this id exists",
              },
              create: {
                name: tag,
                userId: user.id,
        ...
        : undefined,
      sharedWithLists: shareWithLists && shareWithLists.length > 0
        ? {
        ...
        : undefined,
      sharedWith: shareWithEmail && shareWithEmail.length > 0
        ? {
      ...
      hideForecastsUntil: hideForecastsUntil ? new Date(hideForecastsUntil) : undefined,
    },
    include: {
      user: {
        include: {
  ...
  const emailsToNotify = Array.from(new Set([
    ...(shareWithEmail || []),
    ...(userLists.flatMap(l => [l.author.email, ...l.users.map(u => u.email)]) || [])
