...
import { channelVisible, getOrCreateProfile, postEphemeralTextMessage } from '../../lib/_utils_server'
...
export default async function forecast(req : VercelRequest, res : VercelResponse){
  ...
  if (!(await channelVisible(reqbody?.team_id, reqbody?.channel_id))) {
    ...
    await postEphemeralTextMessage(reqbody?.team_id, reqbody?.channel_id, reqbody?.user_id,
    ...
    for (const line of lines) {
      ...
      const forecast = percentageAtEndOfLine ? parseFloat(percentageAtEndOfLine[0]) / 100 : null
