  ...
  return (
    <div className="px-4 pt-12 lg:pt-16 mx-auto max-w-6xl">
      ...
      <div className="prose mx-auto">
        <h4>
          Import all your PredictionBook predictions to Fatebook
        </h4>
        {
          userId ?
...
function PredictionBookImport() {
  ...
  const importFromPredictionBook = api.import.importFromPredictionBook.useMutation({
    async onSuccess() {
      await utils.question.getQuestionsUserCreatedOrForecastedOnOrIsSharedWith.invalidate()
    }
  })
  const deleteAllPredictionBookQuestions = api.import.deleteAllPredictionBookQuestions.useMutation({
    async onSuccess() {
      await utils.question.getQuestionsUserCreatedOrForecastedOnOrIsSharedWith.invalidate()
    }
  })

  const { register, formState: { errors }, handleSubmit } = useForm<z.infer<typeof predictionBookImportSchema>>({
    resolver: zodResolver(predictionBookImportSchema),
  })

  const onSubmit: SubmitHandler<z.infer<typeof predictionBookImportSchema>> = (data, e) => {
      ...
      <form onSubmit={void handleSubmit(onSubmit)}>
        <div className="flex flex-col gap-2">
          ...
          <button
            ...
            disabled={!userId || (importFromPredictionBook.isLoading || Object.values(errors).some(err => !!err))}
          >
          ...
          <button
            className="btn"
            onClick={(e) => {e.preventDefault(); confirm("Are you sure? This can't be undone") && deleteAllPredictionBookQuestions.mutate()}}
